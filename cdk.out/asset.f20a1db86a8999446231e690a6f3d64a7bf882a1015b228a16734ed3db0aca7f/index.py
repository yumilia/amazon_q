...
def _parse_body(event):
    ...
    if "application/json" in ctype or (body.strip().startswith("{") and body.strip().endswith("}")):
        try:
...
def create_transaction(payload, event):
    ...
    now = datetime.now(timezone.utc) if not date_str else datetime.fromisoformat(date_str).astimezone(timezone.utc)
    ym = now.strftime("%Y-%m")
    ts = now.isoformat()
...
def handler(event, context):
    ...
    try:
        ...
        if method == "POST" and path.endswith("/tx"):
            ...
            return {"statusCode": 201, "headers": {"Content-Type": "application/json"}, "body": json.dumps(result, cls=DecimalEncoder)}
        ...
        if method == "GET" and path.endswith("/tx"):
            ...
            return {"statusCode": 200, "headers": {"Content-Type": "application/json"}, "body": json.dumps({"items": res.get("Items", [])}, cls=DecimalEncoder)}
        if method == "GET" and path.endswith("/report/monthly"):
            ...
            report = {"month": month, "total": float(total), "avg_ticket": float(total / count) if count else 0.0, "by_category": {k: float(v) for k, v in totals.items()}, "count": count}
            return {"statusCode": 200, "headers": {"Content-Type": "application/json"}, "body": json.dumps(report)}
        # fallback
        return {"statusCode": 404, "headers": {"Content-Type": "application/json"}, "body": json.dumps({"message": f"Not found: {method} {path}"})}
